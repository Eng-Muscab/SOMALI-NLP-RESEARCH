...
import {
  ...
  ArrowUpRight, RefreshCw, Layers, User,
} from 'lucide-react'
import {
  getAnalyticsOverview, getAnalyticsTrends, getAnalyticsByCategory, getAnalyticsByUser,
} from '@services/analyticsService'
import type { OverviewData, TrendsData, CategoryBreakdown, UserUsage } from '@services/analyticsService'
import { getApiErrorMessage } from '@services/api'
import { useAuth } from '@hooks/useAuth'
import { isAdmin } from '@/types/auth'

const DAYS_OPTIONS = [7, 14, 30, 90]
const GRANULARITY_OPTIONS: { key: 'day' | 'month' | 'year'; label: string }[] = [
  { key: 'day', label: 'Daily' },
...
export default function Analytics() {
  ...
  const fetchAll = async (d = days, g = granularity) => {
    setLoading(true)
    setError(null)
    try {
      const [overviewRes, trendsRes, categoryRes] = await Promise.all([
        getAnalyticsOverview(),
        getAnalyticsTrends(d, g),
        getAnalyticsByCategory(d),
      ])
      setOverview(overviewRes.data)
      setTrends(trendsRes.data)
      setCategories(categoryRes.data.categories)
      if (admin) {
        const userRes = await getAnalyticsByUser(d, 8)
        setTopUsers(userRes.data.users)
      }
    } catch (e) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
            {admin ? 'Platform-wide prediction and usage metrics' : 'Your own prediction and usage metrics'}
          </p>
        </div>
        <button
          onClick={() => fetchAll()}
      ...
      {/* Stat cards */}
      {overview && (
        <div className={`grid gap-4 sm:grid-cols-2 ${admin ? 'lg:grid-cols-4' : 'lg:grid-cols-3'}`}>
          {statCard(admin ? 'Total Predictions' : 'Your Predictions', overview.total_predictions.toLocaleString(), 'All time', <Brain size={18} className="text-primary-600 dark:text-primary-400" />, 'bg-primary-50 dark:bg-primary-500/10', 0)}
          ...
          {admin && overview.total_users !== null &&
      ...
      <div className={`grid gap-6 ${admin ? 'lg:grid-cols-2' : ''}`}>
        ...
        <div className="rounded-2xl border border-neutral-200/60 bg-white p-6 dark:border-neutral-800/60 dark:bg-neutral-900">
          <p className="mb-4 text-xs font-bold uppercase tracking-widest text-neutral-400 dark:text-neutral-500">
            {admin ? 'Top Models' : 'Models You Used'}
          </p>
          {loading ? (
        ...
        {/* User signups — platform-wide, admin only */}
        {admin && (
          <div className="rounded-2xl border border-neutral-200/60 bg-white p-6 dark:border-neutral-800/60 dark:bg-neutral-900">
      ...
      {/* Category breakdown + Most active users (leaderboard is admin only) */}
      <div className={`grid gap-6 ${admin ? 'lg:grid-cols-2' : ''}`}>
        <div className="rounded-2xl border border-neutral-200/60 bg-white p-6 dark:border-neutral-800/60 dark:bg-neutral-900">
          <div className="mb-4 flex items-center gap-2">
            ...
            <p className="text-xs font-bold uppercase tracking-widest text-neutral-400 dark:text-neutral-500">
              {admin ? 'By Category' : 'Your Predictions By Category'}
            </p>
          </div>
          {loading ? (
            <div className="h-52 animate-pulse rounded-xl bg-neutral-100 dark:bg-neutral-800" />
          ) : categories.length === 0 ? (
            <p className="py-10 text-center text-sm text-neutral-400">No category data yet.</p>
          ) : (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={categories} layout="vertical" margin={{ left: 10 }}>
                ...
                <Tooltip contentStyle={{ borderRadius: 12, fontSize: 12 }} />
                <Bar dataKey="total" name="Predictions" fill={CATEGORY_COLOR} radius={[0, 6, 6, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        {admin && (
          <div className="rounded-2xl border border-neutral-200/60 bg-white p-6 dark:border-neutral-800/60 dark:bg-neutral-900">
            <div className="mb-4 flex items-center gap-2">
              ...
              <p className="text-xs font-bold uppercase tracking-widest text-neutral-400 dark:text-neutral-500">Most Active Testers</p>
            </div>
            {loading ? (
              <div className="h-52 animate-pulse rounded-xl bg-neutral-100 dark:bg-neutral-800" />
            ) : topUsers.length === 0 ? (
              <p className="py-10 text-center text-sm text-neutral-400">No testing activity yet.</p>
            ) : (
              <div className="space-y-2.5">
                {topUsers.map((u, i) => (
                  <div key={u.email} className="flex items-center justify-between gap-3 rounded-xl bg-neutral-50 px-3.5 py-2.5 dark:bg-neutral-800/40">
                    <div className="flex min-w-0 items-center gap-2.5">
                      <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-violet-100 text-[10px] font-bold text-violet-700 dark:bg-violet-500/15 dark:text-violet-300">
                        {i + 1}
                      </span>
                      <span className="truncate text-xs font-semibold text-neutral-700 dark:text-neutral-300">{u.email}</span>
                    </div>
                    <span className="shrink-0 text-xs font-bold text-neutral-900 dark:text-white">{u.total} tests</span>
                  </div>
                ))}
              </div>
            )}
      ...
      {overview && (
        ...
        >
          ...
          <p className="text-sm font-medium text-neutral-600 dark:text-neutral-400">
            ...
            {admin && overview.active_users !== null && (
              <>
                {' '}across{' '}
